...
local humidity_noise = {offset = 20, scale = 200, seed = 666, spread = {x = 4000, y = 4000, z = 4000}, octaves = 2, persist = 0.9, lacunarity = 2}
local temperature_noise = {offset = 40, scale = 60, seed = -666, spread = {x = 4000, y = 4000, z = 4000}, octaves = 2, persist = 0.9, lacunarity = 2}
...
local ignore_nodes = {}
ignore_nodes["default:tree"] = true
ignore_nodes["default:jungletree"] = true
ignore_nodes["default:pine_tree"] = true
ignore_nodes["default:acacia_tree"] = true
ignore_nodes["default:aspen_tree"] = true

ignore_nodes["default:leaves"] = true
ignore_nodes["default:jungleleaves"] = true
ignore_nodes["default:pine_needles"] = true
ignore_nodes["default:acacia_leaves"] = true
ignore_nodes["default:aspen_leaves"] = true
        ...
        column = 2
          ...
          elseif underzone.lake and y < underzone.floor + underzone.lake_level then
            data[ivm] = node[underzone.lake]
					elseif not ignore_nodes[minetest.get_name_from_content_id(data[ivm])] then
						data[ivm] = node["air"]
					end
					write = true
				...
				if y > -24610 then --24610 - 3120
					if humid < 25 then -- desert
						...
						elseif temp >= -25 and temp < 0 then -- snow
							if y > -24584 then --24584 - 3120
								if y < -24580 and beaches[cindex] <= 8 then -- 22980-3120
									data[ivm] = node["default:ice"]
									for i = 1, math.random(1,3) do
										data[ivm - area.ystride * i] = node["default:ice"]
									end
					...
					elseif humid >= 25 and humid < 50 then -- tundra
						if temp < -25 then -- iceworld
							if y > -24584 then -- 22984 - 3120
								if y < -24580 and beaches[cindex] <= 8 then
									data[ivm] = node["default:ice"]
									for i = 1, math.random(1,3) do
										data[ivm - area.ystride * i] = node["default:ice"]
									end
						...
						elseif temp >= 0 and temp < 150 then -- tundra
							if y > -24584 then
								if y < -24580 and beaches[cindex] <= 8 then
									...
									if y > -24580 and trees[cindex] > 5 and math.random(1, 500) == 1 then
										table.insert(generate_trees, { { x = pos.x - 4, y = pos.y, z = pos.z - 4 }, minetest.get_modpath("default").."/schematics/acacia_tree.mts"})
									end
								end
							else
								if beaches[cindex] <= 8 then
					...
					elseif humid >= 50 and humid < 125 then -- temperate
						if temp < -25 then -- iceworld
							if y > -24584 then
								if y < -24580 and beaches[cindex] <= 8 then
									data[ivm] = node["default:gravel"]
						...
						elseif temp >= -25 and temp < 25 then -- taiga
							if y > -24584 then
								if y < -24580 and beaches[cindex] <= 8 then
									...
									if y > -24580 and trees[cindex] > 3 and math.random(1, 20) == 1 then
								...
								if y < -24580 and beaches[cindex] <= 8 then
									...
									if y > -24580 and trees[cindex] > 3 and math.random(1, 20) == 1 then
										if math.random(1, 40) == 1 then
											table.insert(generate_trees, { { x = pos.x - 2, y = pos.y, z = pos.z - 2 }, minetest.get_modpath("default").."/schematics/aspen_tree.mts"})
										else
											table.insert(generate_trees, { { x = pos.x - 2, y = pos.y, z = pos.z - 2 }, minetest.get_modpath("default").."/schematics/apple_tree.mts"})
										end
									end
								end
							else
						...
						elseif temp >= 75 then -- jungle
							if y > -24584 then
								if y < -24580 and beaches[cindex] <= 8 then
									...
									if y > -24580 and math.random(1, 10) == 1 then
										table.insert(generate_trees, { { x = pos.x - 1, y = pos.y, z = pos.z - 1 }, minetest.get_modpath("default").."/schematics/jungle_tree.mts"})
									end
								end
							else
								if beaches[cindex] <= 8 then
